...
// ── COMMUNICATION LOG ───────────────────────────────────────────────────────
// Appends a dated line to a job's Communication Log field in Airtable, so the
// assistant (and Job History) can show what has been sent to a client.
async function logCommunication(workerUrl, recordId, existingLog, entry) {
  if (!recordId) return;
  var today = new Date().toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' });
  var line = '[' + today + '] ' + entry;
  var newLog = existingLog ? existingLog + '\n' + line : line;
  try {
    await fetch(workerUrl, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json', 'X-Action': 'update-record' },
      body: JSON.stringify({ id: recordId, fields: { 'Communication Log': newLog } })
    });
  } catch (e) { console.warn('Communication log failed:', e); }
}
